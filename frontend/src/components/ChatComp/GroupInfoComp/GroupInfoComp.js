...
const GroupInfoComp = ({ picClass, groupImg, groupName, onClickHandler, dropDownMenu, toggleDetails, showDetails, detailsSelection, ...props }) => {
    return (
        <div className='container-fluid py-1 bg-grey'>
            <div className='d-flex flex-row align-items-center'>
                {
                    detailsSelection === 'Details' && showDetails ?
                        ...
                        <section className={`${picClass} me-4 pointer`} onClick={() => toggleDetails(detailsSelection)}>
                            <img className='img-fluid' style={{ objectFit: 'cover', width: '100px', height: '75px' }} src={groupImg} alt='Group Pic' />
                        </section>
                }

                {
                    detailsSelection === 'Details' && !showDetails ? (groupName ? <section><p className='d-inline'>{groupName}</p></section> : null) : null
                }
